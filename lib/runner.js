...
class Runner {
    ...
    * processTask(task) {
        ...
        config.logger.debug('Start task "' + task.name + '" with id "' + task.taskId + '"');
        ...
        if ( task.group ) {
            ...
            if ( previousTask && previousTask.processedAt === null ) {
                ...
                if ( previousTask.startAt.getTime() > task.startAt.getTime() ) {
                    ...
                    yield db.rescheduleTask(this._options.collection, task.taskId, new Date(previousTask.startAt.getTime() + 1000));
                }
                return;
            }
        }
        ...
        try {
            ...
            config.logger.error('Task was failed: "' + task.taskId + '"', err.stack);
        ...
        if ( task.repeatEvery > 0 ) {
            ...
            yield scheduler.rescheduleRepeatableTask(this._options.collection, task);
