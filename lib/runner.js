...
module.exports = {
    ...
    run: function* (options) {
        ...
        do {
            previousTask = null;
            taskIndexInCycle++;
            task = yield db.findTaskToProcess(options.collection, options.lockInterval);
            ...
            if ( task.group ) {
                previousTask = yield db.findPreviousTask(options.collection, task);
                ...
                if ( previousTask && previousTask.processedAt === null ) {
                    ...
                    if ( previousTask.startAt.getTime() > task.startAt.getTime() ) {
                        yield db.rescheduleTask(options.collection, task.taskId, new Date(previousTask.startAt.getTime() + 1000));
                    }
                    continue;
                }
            }
            ...
            try {
                ...
                console.log(err);
                yield db.markTaskFailed(options.collection, task.taskId, err.message);
                yield scheduler.rescheduleFailedTask(options.collection, task);
                continue;
            }

            if ( task.repeatEvery > 0 ) {
                // task is repeatable and should be rescheduled
                yield scheduler.rescheduleRepeatableTask(options.collection, task);

            } else {
                // task is not repeatable, finish it and save its result
                yield db.markTaskProcessed(options.collection, task.taskId, taskResult);
            }
