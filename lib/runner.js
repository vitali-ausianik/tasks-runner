...
class Runner {
    ...
    * processTask(task) {
        ...
        try {
            ...
            taskResult = yield Runner.executeTaskProcessor(taskProcessor, task.data, previousTaskResult, extendedInfo);
    ...
    /**
     * Execute task processor (simple function, GeneratorFunction or object with implemented method .run())
     *
     * @param taskProcessor
     * @param data
     * @param previousTaskResult
     * @param extendedInfo
     * @returns {*}
     */
    static * executeTaskProcessor(taskProcessor, data, previousTaskResult, extendedInfo) {
        let context,
            processor,
            result;
        ...
        } else if (typeof taskProcessor === 'object' && typeof taskProcessor.run === 'function') {
            processor = taskProcessor.run;
            context = taskProcessor;

        } else {
            throw new Error(
                'Provided task processor should be function or GeneratorFunction ' +
                'or object with implemented method .run()'
            );
        }

        if (processor.constructor.name === 'GeneratorFunction') {
            result = yield processor.call(context, data, previousTaskResult, extendedInfo);

        } else {
            result = processor.call(context, data, previousTaskResult, extendedInfo);
        }
